...
impl DeepSeekProvider {
    pub fn new(api_key: String, priority: u8) -> Self {
        Self::with_base_url(api_key, "https://api.deepseek.com/v1".to_string(), priority)
    }
        ...
        if !status.is_success() {
            ...
            return Err(match status {
                ...
                StatusCode::SERVICE_UNAVAILABLE | StatusCode::BAD_GATEWAY => {
                    ProviderError::Unavailable("DeepSeek service unavailable".to_string())
                }
                _ => ProviderError::Unavailable(format!("DeepSeek returned HTTP {}", status)),
            });
        }
            ...
            match event_result {
                Ok(event) => {
                    ...
                    match serde_json::from_str::<ChatCompletionChunk>(&event.data) {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "DeepSeek chunk parse error: {}. Data: {}",
                                e,
                                event.data
                            );
                            None
