// tests/v5_1_cli.rs
// Phase V5-1 acceptance tests — `velox` CLI.
//
// Run with: cargo test v5_1_cli
//
// These tests cover:
//   - CLI parser layout (clap subcommand structure)
//   - `velox keys create` actually invokes the admin API end-to-end
//   - `velox config get` actually reads the admin API
//   - `velox migrate status` reads the _sqlx_migrations table
//
// We do not exec the `velox` binary (slower, requires `cargo build`); instead we
...
fn v5_1_cli_help_lists_all_subcommands() {
    ...
    let err = Cli::try_parse_from(["velox", "--help"]).unwrap_err();
...
fn v5_1_cli_no_args_defaults_to_serve() {
    let cli = Cli::try_parse_from(["velox"]).expect("velox with no args should parse");
    assert!(
...
fn v5_1_cli_keys_create_parses_all_flags() {
    let cli = Cli::try_parse_from([
        "velox", "keys", "create", "--name", "prod", "--budget", "100", "--rpm", "60",
    ])
    .expect("must parse");
    match cli.command {
        Some(Command::Keys(velox::cli::keys::KeysCmd::Create {
            name,
            budget,
            rpm,
            tpm,
...
// ── End-to-end tests using a running Velox server ─────────────────────────────

/// `velox keys create` issues a real `POST /admin/keys` against a running Velox
/// and prints the resulting `vx-sk-...` secret. We capture stdout via a piped
...
async fn v5_1_cli_keys_create_invokes_admin_api() {
    ...
    let cmd = velox::cli::keys::KeysCmd::Create {
    ...
    velox::cli::keys::run(cmd, Some(&base_url), Some(&token))
        .await
        .expect("keys create must succeed against admin API");
}

/// `velox config get` issues a `GET /admin/config` against the running server.
#[tokio::test]
async fn v5_1_cli_config_get_reads_admin_config() {
    let base_url = common::spawn_app().await;
    let token = bearer_to_jwt(&common::admin_auth_header(&base_url).await);

    velox::cli::config::run(
        velox::cli::config::ConfigCmd::Get,
        Some(&base_url),
        Some(&token),
    )
    .await
...
/// `velox migrate status` queries `_sqlx_migrations` directly. We just verify it
...
async fn v5_1_cli_migrate_status_reads_migrations_table() {
    common::load_env();
    velox::cli::migrate::run(velox::cli::migrate::MigrateCmd::Status)
        .await
